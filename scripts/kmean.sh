...
for datapath in "${!datapath_method[@]}"; do
    ...
    echo "Apply kmean for '$dataset' 'method'..."
    
    #check if method is empty
    if [ -z "$method" ]; then
        nohup python kmean.py --dataset "$dataset" --data_path "$datapath" > "../log/${dataset}_kmean.out" 2>&1 &
    else
        nohup python kmean.py --dataset "$dataset" --data_path "$datapath" --method "$method" > "../log/${dataset}_kmean.out" 2>&1 &
    fi
    pid=$!
...
for pid in "${!pid_datapath[@]}"; do
    ...
    wait $pid
    return_code=$?
    echo "$datapath:$dataset:$method:$return_code" >> ../log/kmean.out
done